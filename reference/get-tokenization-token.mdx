...
|-----------|------|-------------|
| `format` | string | Optional. Set to `api` to return API-formatted response. Example: `?format=api` |

## Examples

### Request

Make a GET request to the endpoint with your API key in the Authorization header:

<Tabs>
<Tab title="Stage">

```bash
curl -X GET "https://stage.tonder.io/api/v1/tokenization/auth/?format=api" \
...
```bash
curl -X GET "https://app.tonder.io/api/v1/tokenization/auth/?format=api" \
-H "Authorization: Token <YOUR_API_KEY>"
```
...
JWT access token for tokenization service. Use this token immediately to authorize tokenization requests.
</ResponseField>
...
This error occurs when authentication credentials are not provided or are invalid.
...
<Note>
**Translation**: "Authentication credentials were not provided."
</Note>
