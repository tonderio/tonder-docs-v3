...
Make sure you have:
- A Tonder account with Mercado Pago integration enabled
- Your API credentials set up
- A return URL configured for redirect flows
...
## 1. Create a Mercado Pago Payment Request
...
## 4. Redirect Customer to Mercado Pago
...
## 7. Handle Different Payment Outcomes
...
- **Clear branding**: Display Mercado Pago logo to build customer confidence
- **Loading states**: Show progress indicators during redirects
- **Mobile optimization**: Ensure redirect flow works well on mobile devices
- **Popup vs redirect**: Consider using popups for better user experience
</Accordion>

<Accordion title="Technical Implementation">
- **Store transaction ID**: Save the transaction ID before redirecting
- **Timeout handling**: Set reasonable timeouts for payment completion
- **Error handling**: Gracefully handle network issues and API errors
- **Security**: Always verify payment status server-side, not just client-side
</Accordion>
