...
Create and send a POST request to the [Process Transaction](/reference/process-transaction) endpoint to initiate a SafetyPay payment. This request creates a payment voucher that your customer can use at authorized SafetyPay locations:
...
[Webhooks](/direct-integration/webhooks/how-webhooks-works) provide real-time notifications when payment status changes. Set up webhooks to receive payment confirmations:
...
<AccordionGroup>
<Accordion title="Payment Locations">
...
<Accordion title="Processing Timeline">
...
- Explain that SafetyPay is different from OXXO pay.
