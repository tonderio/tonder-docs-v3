...
Before you can process raw card payments, you must obtain a short-lived access token. This token authenticates your requests to the raw card processing endpoint.
...
You will use this `access_token` to authorize your payment request in the next step.

## Step 2: Process the Payment

Send a POST request with raw card data directly to the PCI compliant endpoint. Include the access token from Step 1 in your request.
...
Send a `POST` request to the raw card endpoint with the required parameters below:
