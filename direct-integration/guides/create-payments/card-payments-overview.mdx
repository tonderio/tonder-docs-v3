...
| **Integration Steps** | 3 steps (get token → tokenize → process) | 1 step (process directly) |
...
<Tabs>
  <Tab title="Sandbox">
    | Purpose | Endpoint |
    | --- | --- |
    | Get Access Token | `POST https://stage.tonder.io/api/secure-token/` |
  ...
  <Tab title="Production">
    | Purpose | Endpoint |
    | --- | --- |
    | Get Access Token | `POST https://app.tonder.io/api/secure-token/` |
...
The raw card flow uses a single endpoint for direct processing.
...
sequenceDiagram
    ...
    Client->>Process: 1. Process payment with raw card data
    Process-->>Client: Payment response
```
