...
<Accordion title="Payment Status Response">
...
<Accordion title="Withdrawal Status Response">
...
<Accordion title="Pending OXXO Payment Response">
...
<Accordion title="SPEI Transfers">
...
<Accordion title="Cash Payments (OXXO/SafetyPay)">
...
<Accordion title="Immediate Status Check">

- After receiving an HTTP 202 (Accepted) response
- When `next_action` is present in the response
- For transactions that require additional steps
...
The API includes rate limit information in every response to help you monitor your usage and avoid hitting limits. These headers allow you to implement intelligent polling strategies and backoff mechanisms.
...
- [Set up Webhooks](./webhooks) for real-time status updates
- [Review Error Handling](./error-handling) for comprehensive error management
- [Learn about Payment Methods](./payment-methods) for method-specific status flows
