...
Tonder provides two distinct environments for your integration: **Stage** for development and testing, and **Production** for processing live transactions. Each environment has its own base URL and API credentials.
...
The base URL is the root for all API requests. Append the specific endpoint paths (e.g., `/process/`) to the appropriate base URL.
...
A request to the process endpoint in the **Stage** environment would be:

```POST https://stage.tonder.io/api/v1/process/```

The same request in the **Production** environment would be:
...
- Get your API credentials and follow the [quickstart guide](/direct-integration/quickstart) to make your first transaction
- Learn about [authentication methods](/direct-integration/authentication) to secure your API requests
- Use [testing data](/direct-integration/testing-data) to simulate different payment scenarios
- Review the [process overview](/direct-integration/process-overview) for going live with your integration
