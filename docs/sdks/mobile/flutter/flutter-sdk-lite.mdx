...
- [getBusiness](#get-business): Retrieves all the information about your business.
- [customerRegister](#customer-register): Retrieve a customer's authorization token.
- [createOrder](#create-order): Creates a new order in the system.
- [startCheckoutRouter](#create-payment): Initiates the checkout process for the customer.
- [getVaultToken](#get-vault-token): Retrieves the vault credential that is required to register a customer's card. 
- [registerCustomerCard](#register-customer-card): Registers the customer's card in the vault.
- [getCustomerCards](#get-customer-cards): Retrieves all registered cards for the customer.
- [deleteCustomerCard](#delete-customer-card): Deletes any registered card from the vault.
- [getOpenpayDeviceSessionID](#get-device-session-id): Retrieves your device session id.
