...
Use the `getVaultToken` to retrieve the customer's vault credential that is required to [register a customer's card](#register-customer-card). Use the following example to call this method:
...
Using the customer's token and your skyflow id, you can register the customer's card in the vault. The table below presents the required data, followed by an example call to the `registerCustomerCard` method:
