...
To initialize an instance of Tonder's Ionic SDK, ensure that you have configured it properly. Follow the usage example below:

<Steps>
  <Step title="Import the Lite Checkout class">
    Start by adding the import statement for the `LiteCheckout` class in your file. This is how to do it:
...
| `apiKeyTonder`  | Your Tonder API key. You can find it in your Tonder Dashboard. |
  ...
  <Step title="Start Checkout Router Full method">
    Once you have initialized your `liteCheckout` instance correctly, you need to call the `startCheckoutRouterFull` method and provide all the necessary data as an object. This method handles the entire checkout process, executing the transaction and returning the payment response. Start by calling the method, like exemplified below:
    ...
    | `skyflowTokens` | `string`       | The details of the card to be used on the transaction.        |
    ...
    Below you find also types and an example code for the `startCheckoutRouterFull` parameter:
  ...
  <Step title="Checkout Router Full response">
    ...
    | `provider`           | `string`                   | The name of the payment service provider handling the transaction. |
    ...
    Below you find the type of the response:
