...
| `apiKeyTonder`  | Your Tonder API key. You can find it in your Tonder Dashboard. |
  ...
  <Step title="Start Checkout Router Full method">
    Once you have initialized your `liteCheckout` instance correctly, you just need to call the `startCheckoutRouterFull` method and provide all the necessary data as an object. This method handles the entire checkout process, including executing the transaction and returning the payment response. Start by calling the method, like exemplified below:

    ```js
      liteCheckout.startCheckoutRouterFull({ 
        order, 
        total, 
        customer, 
        skyflowTokens, 
