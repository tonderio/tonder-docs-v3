...
<CardGroup cols={4}>
  ...
  <Card title="Save Customer Card" href="#save-customer-card"/>
...
Using the customer's token and the skyflow id, you can register the customer's card in the vault. The table below presents the required data, followed by an example call to the `saveCustomerCard` method:
...
saveCustomerCardRequest data = saveCustomerCardRequest(
  skyflow_id: '01FGP5H38VZ4B9QPT4T9KJ8J22'
);

Future<saveCustomerCardResponse> saveCustomerCard(String customerToken, saveCustomerCardRequest data) async {
  saveCustomerCardResponse response = await sdkTonder.saveCustomerCard(customerToken, data);
  print(response);
}
```

<Accordion
  title="saveCustomerCardResponse"
>
  ```dart
  class saveCustomerCardResponse {
